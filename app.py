...
# New Secret Key 13 Feb 2022
#####app.config['SECRET_KEY'] = 'b2s5y2LreF4Qr8znxec4345'
...
# New model created 13 Feb - Malaria
#model = load_model('model111_09Mar.h5')
# Note: These models work well now
model = load_model('model111_22Mar.h5')

#####model222=load_model("my_model.h5")
# New Model created 14 Feb - Pneumonia
#model222 = load_model('my_model_09Mar.h5')
# Note: These models work well now
model222 = load_model('my_model_25Mar.h5')
...
# Code from Malaria to use as Pneumonia
@app.route('/upload11', methods=['POST','GET'])
def upload11_file():
...
def ValuePredictor(to_predict_list, size):
    ...
    if(size==30):#Cancer - 30
        loaded_model = joblib.load("model")
        result = loaded_model.predict(to_predict)
    elif(size==8):#Diabetes - 8
        loaded_model = joblib.load("model1")
        result = loaded_model.predict(to_predict)
    #Before: elif(size==11):#Heart
    elif(size==13):#Heart - 13
        loaded_model = joblib.load("model2")
        result =loaded_model.predict(to_predict)
    #elif(size==12):#Kidney Before
    elif(size==19):#Kidney - 19
        #loaded_model = joblib.load("model3")
        loaded_model = joblib.load("model3")
        result = loaded_model.predict(to_predict)
    elif(size==10):#Liver - 10
...
def result():
    if request.method == 'POST':
        ...
        to_predict_list=list(to_predict_list.values())
        to_predict_list = list(map(float, to_predict_list))
        if(len(to_predict_list)==30):#Cancer - 30
            result = ValuePredictor(to_predict_list,30)
        elif(len(to_predict_list)==8):#Diabetes - 8
            result = ValuePredictor(to_predict_list,8)
        #elif(len(to_predict_list)==11):#Heart Before
        elif(len(to_predict_list)==13):#Heart - 13
            result = ValuePredictor(to_predict_list,13)
        #elif(len(to_predict_list)==12):#Kidney Before
        elif(len(to_predict_list)==19):#Kidney - 19
            result = ValuePredictor(to_predict_list,19)
            #if int(result)==1:
            #   prediction ='diabetes'
            #else:
            #   prediction='Healthy'
        elif(len(to_predict_list)==10):#Liver - 10
